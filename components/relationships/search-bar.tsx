...
/**
 * SearchBar — accessible combobox (WAI-ARIA 1.2) over the KB's search engine.
 *
 * Suggestions come from `/api/relationships/search`, which resolves brand,
 * fragrance, alias, partial, and typo. The component holds no fragrance data of
 * its own. Keyboard: ↑ ↓ Enter Esc.
 */
export function SearchBar({
  ...
  const { suggestions, loading } = useRelationshipSearch(query);
          ...
          >
            {suggestions.length === 0 ? (
              <li className="px-4 py-3 text-caption text-muted-foreground">
                {loading
                  ? "Searching…"
                  : "No fragrances found. Try a brand or full name."}
              </li>
            ) : (
              suggestions.map((s, i) => (
                <li
                ...
                >
                  ...
                  <span className="flex shrink-0 items-center gap-2">
                    {/* Surfaced so a typo hit doesn't look like a wrong result. */}
                    {s.matchedOn === "fuzzy" && (
