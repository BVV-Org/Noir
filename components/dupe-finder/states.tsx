    ...
    <div className="mt-3 flex flex-wrap justify-center gap-2">
      {items.map((f) => (
        <button
          ...
          className="h-9 rounded-full border border-border bg-background px-4 text-small font-medium text-foreground transition-colors hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        >
          {f.brand} {f.name}
        </button>
      ))}
...
export function EmptyState({
  ...
  return (
    ...
    >
      <span className="mx-auto grid size-14 place-items-center rounded-full border border-border bg-card text-foreground shadow-card">
        <Sparkles className="size-6" aria-hidden />
      </span>
      <h2 className="mt-6 text-h3">Find its match</h2>
...
export function NoDupesState({
  ...
  return (
    <m.div
      ...
      className="mx-auto max-w-lg py-16 text-center sm:py-24"
    >
      <span className="mx-auto grid size-14 place-items-center rounded-full border border-border bg-card text-muted-foreground shadow-card">
        <SearchX className="size-6" aria-hidden />
      </span>
      <h2 className="mt-6 text-h3">No verified dupe yet</h2>
      <p className="mt-3 leading-relaxed text-muted-foreground">
...
export function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="mx-auto max-w-md py-16 text-center sm:py-24">
      ...
      <h2 className="mt-6 text-h3">Something slipped</h2>
      <p className="mt-3 leading-relaxed text-muted-foreground">
        We couldn&apos;t reach the knowledge base just now. Give it another try.
      </p>
      <button
        onClick={onRetry}
        className="mt-6 inline-flex h-10 items-center justify-center rounded-full bg-primary px-5 text-small font-medium text-primary-foreground transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      >
        Try again
      </button>
    </div>
...
export function LoadingSkeleton() {
  return (
    <div aria-busy="true" aria-live="polite" className="space-y-6">
      ...
      {[0, 1].map((i) => (
        <div key={i} className="space-y-4 rounded-xl border border-border bg-card p-6 shadow-card">
