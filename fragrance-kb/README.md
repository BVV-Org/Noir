...
│   └── community_votes_v1.csv    #   second source (CSV) — demonstrates merge/dedup
...
| `fragrances.json` | Fragrance | brand, name, pyramid (top/heart/base notes), accords, price |
...
- Relationships were **web-verified in July 2026** against fragrance
  ...
  brand/retailer comparison pages, etc.). Each claim carries the specific URL.
- `match.*`, `price.*`, and `performance.*` are **community-reported
  estimates**, not lab measurements. Prices are approximate and drift over time
  — treat `approxPriceUSD` as indicative.
- Weak-consensus pairs were **deliberately omitted** (e.g. Tom Ford Tobacco
  Vanille, Nishane Hacivat, and specific Middle-Eastern Ombre Nomade clones had
  no strong single-clone consensus at build time). Quality over quantity.
...
16 high-confidence relationships across 29 fragrances / 14 brands, spanning
Creed, MFK, Parfums de Marly, Louis Vuitton, Initio, Dior, Chanel, YSL, By
Kilian → Armaf, Maison Alhambra, Fragrance World, Lattafa, Al Haramain. Extend
by adding sources and rebuilding.
